...
class SwiftRLHF(SwiftSft):
    ...
    def _prepare_model_tokenizer(self):
        ...
        if hasattr(args, "reward_model") and args.reward_model is not None:
            rms = (
                args.reward_model
                if isinstance(args.reward_model, list)
                else [args.reward_model]
            )
            num_rms = len(rms)
            rm_types = (
                args.reward_model_type if args.reward_model_type else [None] * num_rms
            )
            rm_revisions = (
                args.reward_model_revision
            ...
            for reward_model_path, rm_type, rm_revision in zip(
                rms, rm_types, rm_revisions
            ):
                args.reward_model = (
                    reward_model_path  # Temporarily set for prepare_single_model
                )
                ...
                if result is not None:
                    ...
                    if args.rlhf_type == "grpo":
                        reward_template = self.args.get_template(
                            processor, processor.model_meta.template
                        )
