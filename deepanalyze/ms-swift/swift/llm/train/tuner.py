...
def get_multimodal_target_regex(
    ...
    *,
    ...
    for module in modules:
        ...
        rejected_pattern = (
            rf'(?!({"|".join(rejected_modules)}))' if rejected_modules else ""
        )
...
def get_vera_target_modules(model, config):
    ...
    if len(set(modules_dict.values())) > 1:
        ...
        if not v:
            raise ValueError(
                "Please manually pass in `vera_target_modules`, do not use `all-linear`,"
        ...
        config.target_modules = [
            t for t in target_modules if any([t in name for name in names])
        ]
    return config


def prepare_adapter(
    args: TrainArguments, model, *, template=None, train_dataset=None, task_type=None
):
    ...
    if args.train_type in ("lora", "longlora"):
        if args.use_swift_lora:
            ...
            lora_config = LoraConfig(
                task_type=task_type, lora_dtype=args.lora_dtype, **lora_kwargs
            )
            ...
            if args.resume_from_checkpoint is None:
                ...
                model = UnslothModel.get_peft_model(
                    ...
                    or 2048,  # 2048 is the default value of unsloth
                    **lora_kwargs,
                )
...
def torchacc_resume_from_checkpoint(args, model):
    ...
    if os.path.isfile(weights_file) or os.path.isfile(safe_weights_file):
        ...
        load_result = load_sharded_checkpoint(
            ...
            strict=False,
            prefer_safe=args.save_safetensors,
        )
...
class TunerMixin:
    ...
    @classmethod
    def prepare_model(
        cls, args, model, *, template=None, train_dataset=None, task_type=None
    ):
        if (
            args.use_liger_kernel
            and "use_liger_kernel"
            not in inspect.signature(TrainingArguments).parameters
        ):
            # Apply liger
        ...
        if args.is_adapter:
            ...
            if args.resume_from_checkpoint:
                ...
                model = tuner.from_pretrained(
                    model, args.resume_from_checkpoint, is_trainable=True, **kwargs
                )
            else:
                if args.train_type in extra_tuners:
                    tuner: Tuner = extra_tuners[args.train_type]
            ...
            freeze_parameters(
                model,
                args.freeze_parameters_ratio,
                args.freeze_parameters,
                args.freeze_parameters_regex,
            )
            if (
                len(args.trainable_parameters) > 0
                or args.trainable_parameters_regex is not None
            ):
                activate_parameters(
                    model, args.trainable_parameters, args.trainable_parameters_regex
                )
