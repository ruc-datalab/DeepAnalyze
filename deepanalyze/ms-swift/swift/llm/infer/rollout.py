...
def llm_worker(
    ...
    while True:
        ...
        if command["type"] in ["call", "fire_and_forget"]:
            ...
            method = getattr(engine, method_name, None) or getattr(
                engine.engine, method_name, None
            )
...
class SwiftRolloutDeploy(SwiftPipeline):
    ...
    def __init__(self, args: Union[List[str], RolloutArguments, None] = None):
        ...
        self.num_connections = (
            1 if self.use_async_engine else self.args.data_parallel_size
        )
    ...
    async def get_world_size(self):
        ...
        """
        return {
            "world_size": self.args.tensor_parallel_size * self.args.data_parallel_size
        }
    ...
    async def infer(
        ...
        *,
        ...
        for i, (connection, requests) in enumerate(
            zip(self.connections, chunked_infer_requests)
        ):
        ...
        all_outputs = [
            output
            for output, requests in zip(all_outputs, chunked_infer_requests)
            if requests
        ]
        all_outputs = list(
...
def run_rollout(args: RolloutArguments, return_url: bool = False):
    if (
        isinstance(args, RolloutArguments)
        and args.__class__.__name__ == "RolloutArguments"
    ):
        deploy_args = args
    else:
...
if (
    not hasattr(WeightSyncWorkerExtension, "old_update_named_param")
    and dtype_annotation == torch.dtype
):
