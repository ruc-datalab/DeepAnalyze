...
def do_sample(model: str, model_type: str, dataset: List[str], iter: int):
    ...
    for device in range(device_count):
        ...
        handler = subprocess.Popen(
            f"{sample_cmd}"
            + f" > logs/sample_iter_{iter}_proc_{device}_cache.log 2>&1",
            env=os.environ.copy(),
            shell=True,
            executable="/bin/bash",
        )
    ...
    for proc, handler in enumerate(handlers):
        ...
        assert os.path.exists(
            os.path.join("sample_output", f"iter_{iter}_proc_{proc}_cache.jsonl")
        )
    ...
    for proc, handler in enumerate(handlers):
        handler.wait()
        assert os.path.exists(
            os.path.join("sample_output", f"iter_{iter}_proc_{proc}_sampling.jsonl")
        ), (
...
def do_train(model: str, model_type: str, datasets: List[str], iter, cmd="sft"):
    ...
    train_cmd = (
        f"{conda_prefix} {gpu_prefix} swift {cmd} "
        f"--model {model} --model_type {model_type} "
...
def do_eval(model, model_type: str, iter):
    ...
    handler = subprocess.Popen(
        f"{eval_cmd}" + f" > logs/eval_iter_{iter}.log 2>&1",
        shell=True,
        env=env,
        executable="/bin/bash",
    )
