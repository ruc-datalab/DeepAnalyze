...
async def chat_completions(
    ...
    # Create client dynamically for each request, using the user-provided api_key (if provided)
    if api_key:
        vllm_client = openai.OpenAI(base_url=API_BASE, api_key=api_key)
        vllm_client_async = openai.AsyncOpenAI(base_url=API_BASE, api_key=api_key)
    else:
        # If not provided, use default value (e.g., from environment variables)
        vllm_client = openai.OpenAI(base_url=API_BASE)
        vllm_client_async = openai.AsyncOpenAI(base_url=API_BASE)
